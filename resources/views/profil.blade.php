...
@section('contenu')
    ...
    <div class="container mt-5">
        <div class="row justify-content-center">
                <img class="border border-info img-fluid shadow rounded-circle" src="{{asset($user->imgprofil)}}" alt="Image de profil" style="with:180px; height:180px;">
        </div>
        ...
        <div class="container">
            <div class="row">
                ...
                @isset($userAuth)
                    <div class="col-md-2  text-center p-4">
                        @if($userAuth->subscription !== 1)
                            <a href="{{route('user-sub')}}" class="form-control btn btn-dark">S'abonnez!</a>
                        @else
                            <a href="{{route('user-unsub')}}" class="form-control btn btn-outline-dark">Se désabonnez
                                :(</a>
                        @endif
                    </div>
                @endisset
                        ...
                        @if($user->follow > 0)
                            <a href="{{route('unfollow', ['slug' => $user->slug])}}"
                               class="form-control btn btn-danger">
                                Arrêter de suivre<br> {{$user->firstname}} {{$user->name}}
                            </a>
                        @else
                            <a href="{{route('follow', ['slug' => $user->slug])}}" class="form-control btn btn-success">
                                FOLLOW ME !!!!
                            </a>
                        @endif
                    </div>
                @endempty
                    ...
                    <div class="col-md-7 text-right p-4">
                        @isset($userAuth)
